...
请直接返回JSON，不要包含其他内容。"""
    ...
    async def _organize_memory_points(self, participants: List[str], summary: str, original_text: str):
        ...
        prompt = f"""你是一个对话分析助手, 专门为每位参与者整理「长期人物记忆」。
长期人物记忆指: 在较长时间内稳定、可复用的客观信息, 以简短的事实条目形式保存, 方便后续快速检索。

[对话信息]
参与者: {filtered_participants}
话题概括: {trimmed_summary}
补充原文片段(可为空, 可忽略细节): {trimmed_original}
...
记忆内容风格要求:
1. 使用简短、客观的事实描述, 不要使用聊天语气或主观评价。
2. 每条记忆尽量控制在 10~30 个汉字, 最多不超过 50 个汉字。
3. 优先使用名词短语或简短陈述句, 避免长句和多层转折。
4. 不要出现感叹句、语气词或泛泛而谈的风格描述, 例如:
   - 避免: "沟通风格轻松幽默"、"偏好荒诞反差类内容"、"围观心态"
   - 推荐: 用更具体的行为或偏好表达, 如: "常用吃瓜表情包回复消息"。
5. 不要以「在群聊中」「最近」「近期」等时间/场景开头, 直接描述稳定事实:
   - 避免: "近期持续进行《泰拉瑞亚》仓库区任务, 有与雇佣兵对抗的战斗经验"
   - 推荐: "擅长《泰拉瑞亚》仓库区任务与雇佣兵战斗"。

判定「值得记录的长期人物记忆」的原则:
1. 只保留对未来多次对话有帮助的稳定信息, 例如:
   - 长期喜好/厌恶(游戏类型, 食物, 兴趣等)
   - 长期习惯(作息, 常用平台/设备, 典型行为方式)
   - 性格特征, 价值观, 说话方式(如: 常用某类表情包)
   - 能力与限制(擅长/不擅长的事情, 硬件/网络等长期限制)
   - 持续性的项目或状态(正在进行的长期计划, 而不是一次性的事件)
2. 不要记录一次性或短期事件, 例如:
   - 某一局/某一天的游戏输赢, 卡顿, 崩溃, Bug 细节
   - 某次临时的技术问题排查过程
   - 临时情绪, 随口吐槽, 单次观点(例如: "这局游戏优化好差" "刚刚好卡")
   - 只描述当前时间点、很快会失效的信息(如: "今晚要睡觉了")
3. 可以将多条相关聊天内容合并成一条更抽象的记忆, 例如:
   - 多次聊到不同游戏的卡顿问题, 可抽象为"经常遇到游戏性能问题"
4. 必须参考「现有人物记忆」, 如果新信息和已有内容几乎相同, 则不要重复生成;
   如发现明确变化(例如: 从"电脑只有 8G 内存"变成"已升级到 32G 内存"),
   可以新增一条反映变化的记忆。
5. 如果本次对话中没有任何符合以上条件的新长期人物记忆, 请返回空数组 []。

只返回 JSON 数组本身, 不要输出任何额外说明、自然语言解释或 Markdown 代码块。
"""
